                        ...
                        <tr>
                          <td style="text-align: center;">
                            <div class="text-author">
                              ...
                              <p class="position for-space">キャンセル料：${price}円</p>
                              <p class="position for-space">住所：${storeInfo.address}</p>
                              <p class="position for-space">電話：${storeInfo.tel}</p>
                             </div>
                          </td>
                        </tr>
                      </table>
                ...
                <table align="center" role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin: auto;">
                  ...
                  <tr>
                    <td class="bg_light" style="text-align: center;">
                      <p>If you have no idea about this email, just ignore, thank you.</p>
                    </td>
                  </tr>
                </table>
...
const createMail = (contactEmail, customerEmail, html) => {
  return {
...
exports.handler = async (event) => {
  ...
  const { contactEmail, customerInfo } = event.body;
  const html = createHTML(event.body);

  try {
    const transporter = nodemailer.createTransport(createOptions());
    const data = await getCustomer(customerInfo.customerId);
    const customerEmail = data.Item.email;
    const mail = createMail(contactEmail, customerEmail, html);
    const result = await transporter.sendMail(mail);
    responseBody = event.body;
    statusCode = 200;
    message = `Successfully sent a email. RESULT=${result}`;
  } catch (err) {
    message = `Failed to send a email. ERROR=${err}`;
    statusCode = 403;
  }
